...
<?php } else {?>
<div class="eight columns shipping shipping-pixel">
	<h3><?php
		if ( $show_package_details ) {
			printf( __( 'Calculate Shipping #%d', 'woocommerce' ), $index + 1 );
		} else {
...
<p class="form-row form-row-wide">
				<select name="shipping_method[<?php echo $index; ?>]" data-index="<?php echo $index; ?>" id="shipping_method_<?php echo $index; ?>" class="shipping_method country_to_state">
					<?php foreach ( $available_methods as $method ) : ?>
						<option value="<?php echo esc_attr( $method->id ); ?>" <?php selected( $method->id, $chosen_method ); ?>><?php echo wp_kses_post( wc_cart_totals_shipping_method_label( $method ) ); ?></option>
					<?php endforeach; ?>
				</select>
